 ...
 * day rather than one per trading session. That changes nothing about the query
 * — the newest timestamp is still the newest bar, so this is the crypto query's
 * shape unchanged — but it is why the CTE the stock markets call `session` is
 * `newest` here.
 *
 ...
 * upstream loader is behind, that is Friday's bar and not an empty one. There
 * is no index symbol to exclude the way the PH query must: this database holds
 * pairs and nothing else, metals quoted as pairs (`XAUUSD`) included.
 *
...
export async function listDailyMovers(): Promise<DailyMovers> {
  ...
  const { rows } = await forexDb().query<MoverRow>(DAILY_MOVERS_SQL, [
    MOVER_LIMIT,
  ]);
