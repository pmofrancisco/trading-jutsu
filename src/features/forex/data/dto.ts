...
/**
 * One pair's move over the latest daily bar.
 *
...
export interface DailyMover {
  symbol: string;
  /**
   * The pair's logo, as an absolute URL. Built from the symbol rather than
  ...
  close: number;
  /**
   * The move from the symbol's own previous close — in the pair's quote
   * currency, and as a percentage of it. The previous close itself is not here:
   * it is what the figures are measured from, not something the tables show.
   */
  change: number;
 ...
 * move. Pairs that closed unchanged appear in neither list — they moved in no
 ...
 * the day, not every pair that moved over it.
 */
export interface DailyMovers {
   ...
   * of the board still is — fifteen of the seventy-one pairs today, the crosses
   * rather than the majors. Carried once here rather than on every row, because
   * it is one value per market and a hundred rows repeating it would be the
   * same string a hundred times over the wire.
   */
...
/**
 * One pair's standing in a period's ranking.
 *
 ...
 * the period began to measure it against, so a pair that cannot be priced over
...
export interface PeriodLeaders {
  /**
   * The day every ranking ends at, or `null` when no pair in the latest day
